...
$return = array();
$return['result'] = false;
$return['info'] = "cancel_sale_reports.php: ";
    ...
    if(isset($_POST['action']) && !empty($_POST['action'])){
        if(isset($_POST['type']) && !empty($_POST['type'])){
            ...
            if($action == "fetch_all"){
                ...
                $where_clause = array("is_updated=1");
                if($start && $end){
                    $where_clause = "date BETWEEN '$start 00:00:00' AND '$end 23:59:59' AND `is_updated`='1'";
                }

                $extra_columns = array();
                if($type == "overview"){
                    $extra_columns = array("no_of_items", "no_of_units", "making_cost", "(`total_price` - `making_cost`) AS `profit`", "total_price", "offer_percentage", "offer_amount");
                }elseif($type == "summary"){
                ...
                $where_clause = "`invoice_id` LIKE '$invoice_id' AND `is_updated`=1";
                //$where_clause = array("invoice_id=$invoice_id", "is_updated=1");
