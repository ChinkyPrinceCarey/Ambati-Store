...
$(function(){
    ...
    dump_stock_btn = $("#dump_stock_btn");
    
    initLoadStock("fetch_all", "stock", function(stock_data){
        unscanned_data.data = stock_data;
        initLoadStock("summary_data", "stock", function(stock_summary){
            unscanned_data.summary = stock_summary;
            initLoadStock("billing_data", "stock", function(stock_billing){
                unscanned_data.billing = stock_billing[0];
    ...
    dump_stock_btn.on('click', function(){
        if(!unscanned_data.data.length){
            smallModal(
                "No Stock to Dump", 
                "It seems all items are scanned from stock",
                [
                    {
    ...
    scanner_data = {
        ...
        offer_dialogue(){
            offer_dialogue(
                ...
                unscanned_data, 
                false,
                {title: "Verify and Confirm Dump Stock", desc: "Dump Stock Overview", primary_btn_title: "Dump Stock"}
            );
        },
        evaluateOffer(input_id){
            evaluateOffer(input_id, unscanned_data);
        },
        remove_offer(){
...
function dump_stock(){
    data_param = {
        action: "dump_stock",
        data: JSON.stringify(unscanned_data.data)
    }
    ...
    ajaxPostCall('lib/warehouse_stock_reports.php', data_param, function(response){
        ...
        if(response.status){
            ...
            smallModal(
                ...
                `
                <p>Total No.# of items are dumped <b>${unscanned_data.data.length}</b></p>
                <table>
                    <tr>
                        <td>Total Price of</td>
                        <td><b>${unscanned_data.billing.total.toFixed(2)}</b></td>
                    </tr>
                    <tr>
                        <td>Total Making Cost of</td>
                        <td><b>${unscanned_data.billing.making_cost.toFixed(2)}</b></td>
                    </tr>
                    <tr>
                        <td>Profit of</td>
                        <td><b>${(unscanned_data.billing.total - unscanned_data.billing.making_cost).toFixed(2)}</b></td>
                    </tr>
                </table>
                `,
                [
