...
if(isset($_POST['data']) && !empty($_POST['data'])){
    if(isset($_POST['action']) && !empty($_POST['action'])){
        ...
        if($action == "create"){
            $fields_def = array(NAME, EMAIL_ID, MOBILE_NUMBER, ROLES);
            $fields_data = validate_fields($_POST['data'][0], $fields_def);

            if($fields_data['result']){
                $default_where_column = "email_id";
                $default_where_value = $fields_data['data']['email_id'];
                $default_where = array("$default_where_column=$default_where_value");
                ...
                $query_values = array(
                                    ...
                                    $fields_data['data']['email_id'],
                                    $fields_data['data']['mobile_number'],
                                    json_encode($fields_data['data']['roles']),
            ...
            if($fields_data['result']){
                ...
                foreach($fields_data['data'] as $set_column => $set_value){
                    if($set_column == "roles") $set_value = json_encode($set_value);
